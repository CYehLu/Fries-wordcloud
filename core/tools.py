...
def get_common_words():
    ...
    """
    with open(PATH + 'common_words.txt', encoding='utf-8') as file:
...
def get_all_userid():
    ...
    """
    with open(PATH + 'all_userid.txt') as file:
...
def get_content_from_sql():
    ...
    Return all contents
    """
    conn = sqlite3.connect(PATH + 'fries.sqlite')
    c = conn.cursor()
...
def plot_wordcloud(content_cut, filename):
    ...
    """
    plt.figure()
    plt.imshow(WordCloud(font_path=PATH+'ext/jf-openhuninn-1.1/jf-openhuninn-1.1.ttf').generate(' '.join(content_cut)))
    plt.axis('off')
